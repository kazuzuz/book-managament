...
function addFavorite(bookId) {
  ...
  fetch(`/api/books/${bookId}/add_favorite`, {
    ...
    if (response.ok) {
      addFavoriteBtn.setAttribute("hidden", "true");
      deleteFavoriteBtn.removeAttribute("hidden");
    }
    //async/await
  });
}
...
function deleteFavorite(bookId) {
  ...
  fetch(`/api/books/${bookId}/delete_favorite`, {
    ...
    if (response.ok) {
      addFavoriteBtn.removeAttribute("hidden");
      deleteFavoriteBtn.setAttribute("hidden", "true");
    }
    //async/await
  });
}
